  ...
  bool loop(void) override
  {
    ...
    if (tp.state)
    {
      ...
      { /// スクロールエリア内でのタッチ操作の処理;
        ...
        if (tp.wasClicked())
        { /// タップ操作でチャンネル選択;
          int new_channel_index = (tp.x / channel_item_width) + ((tp.y + scroll_y) / channel_item_height) * channel_item_cols;

          if (new_channel_index < 0) { new_channel_index = 0; }
          else if (new_channel_index >= enable_channel_count) { new_channel_index = enable_channel_count - 1; }
          else
          if (target_channel_index != new_channel_index)
          {
      ...
      if (tp.base_x >= scroll_width && tp.base_y < M5.Display.height())
      {
    ...
    case mode_channel_select:
      {
        ...
        if (new_channel_index != target_channel_index)
        {
          ...
          if (new_channel_index >= enable_channel_count)
          {
    ...
    if (scroll_y_add)
    {
      ...
      if (new_y < 0) { new_y = 0; }
      static constexpr int scroll_liimt = channel_item_height * ((511 + channel_item_cols) / channel_item_cols) - scroll_height;
      if (new_y > scroll_liimt) { new_y = scroll_liimt; }
      if (scroll_y != new_y)
      {
        scroll_y = new_y;
        updateDisplay(true);
...
private:
  ...
  static constexpr uint8_t channel_item_width = scroll_width / channel_item_cols;
  ...
  {
    int new_y = (target_channel_index / channel_item_cols) * channel_item_height;
    if (scroll_y > new_y)
    {
      scroll_y_add = 0;
  ...
  {
    M5.Display.fillRect(scroll_width, 0, 320 - scroll_width, M5.Display.height(), TFT_BLACK);
    M5.Display.fillRect(scroll_width, scroll_height, 1, M5.Display.height() - (scroll_height<<1), TFT_DARKGRAY);
    M5.Display.pushImage(scroll_width + 8, 120, logo_sender_width, logo_sender_height, (const m5gfx::rgb565_t*)logo_sender);
  }
  ...
  void drawUIValueUpDown(int sign = 0)
  {
    ...
    // M5.Display.drawRoundRect(x,              2, w, slider_btn_height - 4, 4, sign > 0 ? TFT_WHITE : TFT_DARKGRAY);
    M5.Display.drawRoundRect(x, slider_y -       10 - (slider_btn_height-4), w, slider_btn_height - 4, 4, sign > 0 ? TFT_WHITE : TFT_DARKGRAY);
    M5.Display.drawRoundRect(x, slider_y + 128 + 10                        , w, slider_btn_height - 4, 4, sign < 0 ? TFT_WHITE : TFT_DARKGRAY);
  }
  ...
  void drawUIValueSet(uint16_t new_value, bool force_redraw = false)
  {
    ...
    if (force_redraw)
    {
      M5.Display.fillRect(scroll_width + 1, 0, 320 - scroll_width, M5.Display.height(), TFT_BLACK);
      py = 128 + slider_y;

      M5.Display.setColor(TFT_WHITE);
      drawFocusBox(&M5.Display, slider_x-8, slider_y - 5, slider_w + 16, 128 + 10, 2);

      fillBar(&M5.Display, slider_x, slider_y, slider_w, y - slider_y, target_channel);

      int text_x = (scroll_width + M5.Display.width()) >> 1;
      M5.Display.drawString("+", text_x, slider_y -       8 - (slider_btn_height >> 1));
      M5.Display.drawString("-", text_x, slider_y + 128 + 8 + (slider_btn_height >> 1));
  ...
  void fillBar(LovyanGFX* gfx, int32_t x, int32_t y, int32_t w, int32_t h, size_t ch = 0)
  {
    ...
    uint32_t prev_color = (color_add + getBarColor(y-slider_y)) & 0xF8FCF8u;
    uint32_t py = y;
    while (h--)
    {
      uint32_t color = (color_add + getBarColor(++y-slider_y)) & 0xF8FCF8u;
  ...
  void updateDisplay(bool full_redraw = false)
  {
    ...
    for (int ch = 1; ch < DMX_MAX_PACKET_SIZE; ++ch)
    {
      if (!visible[ch])
      {
        if (data[0][ch] == data[1][ch]) { continue; }
        visible[ch] = true;
        full_redraw = true;
      }
      ++drawcount;
      ...
      if (full_redraw || current_data != prev_data)
      {
        ...
        if (y + channel_item_height > 0)
        {
          ...
          if (ch < DMX_MAX_PACKET_SIZE)
          {
            ...
            {
              int fw = (ui_mode == ui_mode_t::mode_channel_select) ? 2 : 1;
              c.setColor(0xFFFFFFu);
              drawFocusBox(&c, 0, 0, channel_item_width, channel_item_height, fw);
            }
            bool dimmer = (ui_mode != ui_mode_t::mode_channel_select) && (ch != target_channel);
            ...
            c.fillArc(circle_x, circle_y, 17, 15, 270 + angle, 269, dimmer ? 0x131313u : 0x333333u);
            uint32_t color = bar_color_table[ch & 15];
            if (dimmer)
            {
              color = (0x3F3F3F & (color >> 2)) + 0x2F2F2Fu;
            }
            c.fillArc(circle_x, circle_y, 19, 13, 270, 270 + angle, color);
          }
