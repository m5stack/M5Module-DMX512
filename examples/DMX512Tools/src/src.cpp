...
static ui_button_t btns[2] = { { 0, 200, 120, 40, "Receiver" }, { 200, 200, 120, 40, "Sender" } };
...
void drawSelectSetup(void)
{
  ...
  M5.Display.drawString("DMX512Tools", M5.Display.width()>>1, M5.Display.height()>>2);

  M5.Display.setFont(&fonts::AsciiFont8x16);
  M5.Display.drawString("Select mode", M5.Display.width()>>1, M5.Display.height()*12>>4);
...
void setup(void)
{
  ...
  dmx_driver_install(dmxPort, DMX_MAX_PACKET_SIZE, dmxQueueSize, &queue, dmxInterruptPriority);
...
int getBtnIndex(int x, int y)
{
  for (int i = 0; i < 2; ++i)
  {
    if (btns[i].contain(x, y)) { return i; }
  }
  return -1;
}
