...
public:
  void setup(void) override
  {
    ...
    M5.Display.drawFastHLine(0, CONSOLE_Y - 3, M5.Display.width(), TFT_DARKGRAY);

    canvas.setColorDepth(8);
    canvas.createSprite( M5.Display.width(), M5.Display.height() - CONSOLE_Y);
  ...
  bool loop(void) override
  {
    ...
    if (!change_view)
    {
      ...
      change_view = (td.wasClicked() && td.y > CONSOLE_Y && td.y < M5.Display.height());
    }
    if (change_view)
    {
    ...
    if (xQueueReceive(queue, &packet, DMX_RX_PACKET_TOUT_TICK))
    {
      if (packet.status == DMX_OK)
      {
        ...
        if (ESP_OK == dmx_read_packet(dmxPort, data[data_idx], packet.size))
        {
  ...
  void updateDisplay(int32_t duration)
  {
    ...
    for (int ch = 1; ch < DMX_MAX_PACKET_SIZE; ++ch)
    {
      ...
      if (full_redraw || current_data != prev_data)
      {
        ...
        int py = BAR_HEIGHT - (full_redraw ? 0 : ((prev_data + 1) >> 1));
        if (full_redraw)
        {
      ...
      if (graph_view)
      {
        ...
        canvas.fillRectAlpha(32 + x1, canvas.height()-1, x2 - x1 + 1, 1, 128, bar_color_table[ch & 15]);
      }
      else
      {
        if (prev_data != current_data)
        {
          ...
          {
            canvas.println();
            canvas.setCursor(9 * canvas.fontWidth(), canvas.getCursorY());
          }
