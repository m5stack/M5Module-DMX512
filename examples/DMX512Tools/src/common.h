...
static constexpr uint32_t bar_color_table[] = { 0xED7F11, 0xCDB001, 0xA0D904, 0x6EF51A, 0x3FFF3F, 0x1AF56E, 0x04D9A0, 0x01B0CD, 0x117FED, 0x314EFD, 0x5E25FA, 0x9009E4, 0xBF00BF, 0xE40990, 0xFA255E, 0xFD4E31 };
...
struct ui_rect_t
{
  ...
  int w = 0;
  int h = 0;
  ui_rect_t(int src_x, int src_y, int src_w, int src_h) : x { src_x }, y { src_y }, w { src_w }, h { src_h } {};
  bool contain(int src_x, int src_y) { return src_x >= x && src_x < x + w && src_y >= y && src_y < y + h; }
};

struct ui_button_t : public ui_rect_t
{
  ui_button_t(int src_x, int src_y, int src_w, int src_h, const char* src_caption) : ui_rect_t { src_x, src_y, src_w, src_h }, caption { src_caption } {};

  const char* caption = nullptr;
  void draw(LovyanGFX* gfx, bool has_focus, bool is_pressed, bool invalidated = false)
  {
